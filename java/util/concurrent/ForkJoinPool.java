...
public class ForkJoinPool extends AbstractExecutorService {
     ...
     * 完整版的 externalPush，处理一些初始化之类的 case
     * @param task the task. Caller must ensure non-null.
     */
    ...
    final void externalPush(ForkJoinTask<?> task) {
        ...
        int rs = runState;
        if ((ws = workQueues) != null && (m = (ws.length - 1)) >= 0 && // 外部提交的task，肯定会到偶数位下标的队列上
            (q = ws[m & r & SQMASK]) != null && r != 0 && rs > 0 &&    // SQMASK = 0x007e = 0b0111_1110，任何数和 SQMASK 进行 & 运算 都会是偶数
            U.compareAndSwapInt(q, QLOCK, 0, 1)) {        // 队列上锁
            ForkJoinTask<?>[] a; int am, n, s;
            if ((a = q.array) != null &&
                (am = a.length - 1) > (n = (s = q.top) - q.base)) {
                int j = ((am & s) << ASHIFT) + ABASE;
                U.putOrderedObject(a, j, task);                        // 把 task 放到队列的 top 端
                U.putOrderedInt(q, QTOP, s + 1);
