...
final class SortedOps {
    ...
    private static final class IntSortingSink extends AbstractIntSortingSink {
        ...
        public void end() {
            ...
            Arrays.sort(ints);                                      // 对元素做排序
            downstream.begin(ints.length);                          // 调用下游方法的 begin
            if (!cancellationRequestedCalled) {
                for (int anInt : ints)
                    downstream.accept(anInt);                       // 调用下游方法的 accept
            }
            else {
                for (int anInt : ints) {
                    if (downstream.cancellationRequested()) break;
        ...
        @Override
        public void accept(int t) {
            b.accept(t); // 可以看到，对于 sort 类型的操作，它的实际操作没有调用 downstream
        }                // 它内部是先将所有元素做了存储，在调用 end 方法的时候再调用 downstream
    }
