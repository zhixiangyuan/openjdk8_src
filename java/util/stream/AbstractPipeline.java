...
abstract class AbstractPipeline<E_IN, E_OUT, S extends BaseStream<E_OUT, S>>
     ...
     * stage). 反向链接到管道链的头部（如果这是源阶段，则为它本身）。
     */
    @SuppressWarnings("rawtypes")
    private final AbstractPipeline sourceStage;

    /**
     * The "upstream" pipeline, or null if this is the source stage. 上游管道，如果是第一次创建流则为 null。
     */
    @SuppressWarnings("rawtypes")
     ...
     * pipeline object. 此管道对象中表示的中间操作的操作标志。
     */
     ...
     * null. 源分裂器。仅对头管道有效。如果非 null，则在使用管道之前，{@code sourceSupplier} 必须为 null 并且在在使用管道之后将其设置为 null。
     */
     ...
     * valid for the source stage. 是否为并行流
     */
     ...
     * @param sourceFlags the source flags for the stream source, described in       流源的原标志
     * {@link StreamOpFlag}
     * @param parallel {@code true} if the pipeline is parallel                      是否为并行流
     */
