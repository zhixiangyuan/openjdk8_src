...
public final class Spliterators {
    ...
    static final class IntArraySpliterator implements Spliterator.OfInt {
        ...
        @Override
        public OfInt trySplit() { // 作用便是将数组一分为二，返回的是前一半的数组，当前数组变成后一半
            int lo = index, mid = (lo + fence) >>> 1; // (start + end)/2
            return (lo >= mid)
                   ? null
        ...
        @Override
        public boolean tryAdvance(IntConsumer action) { // 遍历数组
            if (action == null)
                throw new NullPointerException();
            if (index >= 0 && index < fence) {
                action.accept(array[index++]); // index++，遍历当前元素并且将下标加一
                return true;
            }
            return false;
        }
        ...
        @Override
        public long estimateSize() { return (long)(fence - index); } // 计算剩下的元素
